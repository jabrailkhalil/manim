...
representation of the Scene. Manager requests a fresh top-left-origin, C-contiguous
``uint8`` RGBA array, advances its Cairo sample clock, and delivers that array to its
:class:`.SceneFileWriter`. Drawing/readback itself does not advance time. This concludes one iteration of the
...
- Then Manager asks the renderer to do its job. The only mobject that needs to
  ...
  renderer exposes readback; Manager delivers the owned array to the file writer.
