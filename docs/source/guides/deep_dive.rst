...
After drawing, the manager requests a copy of the renderer's pixels as a
``uint8`` RGBA array, with row zero at the top. It advances the animation clock
by one frame interval, then passes the array to the :class:`.SceneFileWriter`.
...
- Then Manager asks the renderer to do its job. The only mobject that needs to
  ...
  The camera determines the view, and Cairo draws the current mobject into an
  image buffer. The manager advances the clock and sends a copy of the pixels
  to the file writer.
